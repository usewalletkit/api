...
<Steps>
    ...
    <Warning>
    You must never store the `user in` on your servers.
    </Warning>
    </Step>
    ...
    `ownerId` is automatically set to the user id when using WalletKit SDKs, you can safely ignore it in this case.
